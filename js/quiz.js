...
//--------Intro--------------------------------------------------------    
    ...
    //<-----Array med intro tekster----->
        const introTekster = [
          ...
          `<span class="linje">Kig op. Portrætterne omkring dig stirrer, alle øjne er rettet mod dig. 
        Hvem ser de? Hvem er du?</span> 
        <span class="linje">Museets kunstnere malede deres indre. Deres frygt, drømme, tanker og splittelse. 
        Mange mennesker skjuler tanker om sig selv. Tanker de tror, de står alene med. </span>
        ...
        //Vi skal lave en funktion som viser vores introtekster, denne funktion kalder vi for visIntro()
            function visIntro() {
                ...
                aktiveTimeouts.push(setTimeout(() => {
                  ...
                  //Vi vil have lavet sådan at hver af de to intro tekster for hver deres class navn, så vi kan style dem forskelligt inde i css

                  if (introStep === 0) {
                    //Denne sætning siger at hvis introStep er lig med 0 både datatypen og værdien (0 er index tallet)
                    inaktivTekst.classList.add("introEt"); //Så tilføjer vi classen intro´et
                  }
...
//--------Wordcloud-------------------------------------------------------- 
    ...
    //<-----Wordcloud funktionen----->
        //Her laver vi så at wordclouden fungerer. For hver gang et ord gentages forstørres det. Det ord der er svaret flest gange i midten af ordskyen
        ...
        function visWordcloud() {
            ...
            //-----Sortering af ordenes optælling i funktionen i funktionen-----
                ...
                for (let ord in count) {
                  ...
                  ordArray.push({ tekst: ord, antal: count[ord] }); //Pusher den hvert ord ind i vores ordArray, her laver den et lille objekt for hvert ord, hvor den tilføjer attributterne tekst og antal
                }
            ...
            //-----Udseendet af wordcloud ordene i funktionen-----
                ...
                centreretArray.forEach((ordObj) => { //Her bygger vi selve wordclouden ved at lave et loop i det centreret array
                    ...
                    span.style.margin = "6px 12px"; //Her tilføjer vi afstand uden om hvert ord, så de ikke klumper sig sammen
            ...
            //  //<-----Reset knap----->
            //Vores reset knap som kan rydde wordcloud, men lige nu er den inaktiv
...
//--------Flow--------------------------------------------------------  
    ...
    //<-----Flowet----->
        ...
        function startFlow(event) {  //Her har vi lavet en funktion startFlow, som tager event som parameter, så vi kan tjekke hvad der bliver klikket på
            ...
            //-----Intro start-----
                if (introStep < introTekster.length - 1) { //Hvis introStep er mindre end længden af arrayet introTekster
